...
public class MysqlConnetionPool {
  ...
  public static synchronized Connection getConnection() {// 共享资源
  ...
  private static ConnectionWrapper getNewConnection() {
    try {
      Connection con = (Connection) DriverManager.getConnection(m_url, m_user, m_password);
  ...
  private static void clearClosedConnection() {
    ...
    if (time - m_lastClearClosedConnection < CHECK_CLOSED_CONNECTION_TIME) { return; }
    ...
    if (m_notUsedConnection.size() < decrease) { return; }
  ...
  public synchronized static void printDebugMsg(PrintStream out) {
    ...
    msg.append("debug message in " + MysqlConnetionPool.class.getName());
    msg.append("\r\n");
...
class ConnectionWrapper implements InvocationHandler {
  ...
  ConnectionWrapper(Connection con) {
    this.connection = (Connection) Proxy.newProxyInstance(con.getClass().getClassLoader(),
  ...
  public Object invoke(Object proxy, Method m, Object[] args) throws Throwable {
    ...
    if (CLOSE_METHOD_NAME.equals(m.getName())) {
      MysqlConnetionPool.pushConnectionBackToPool(this);
    } else {
