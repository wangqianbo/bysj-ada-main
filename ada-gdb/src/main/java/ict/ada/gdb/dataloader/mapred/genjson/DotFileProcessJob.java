 ...
 * 
 */
public class DotFileProcessJob extends Configured implements Tool {

  private static Log LOG = LogFactory.getLog(DotFileProcessJob.class);

  @Override
  public int run(String[] arg0) throws Exception {
    if (arg0.length != 1) {
      LOG.fatal("need one argument: Input Dot Text file path");
      return -1;
    }
    String inputfolder = arg0[0];
    Job job = new Job(getConf(), "Dot Text File Process:" + inputfolder);

    job.setJarByClass(DotFileProcessJob.class);
    job.setNumReduceTasks(0);// no Reducer
    job.setMapperClass(DotFileProcessMapper.class);
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new DotFileProcessJob(), args);
  }
