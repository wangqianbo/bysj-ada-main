 ...
 * 
 */
public class EdgeFileProcessJob extends Configured implements Tool {

  private static Log LOG = LogFactory.getLog(EdgeFileProcessJob.class);

  @Override
  public int run(String[] arg0) throws Exception {
    if (arg0.length != 1) {
      LOG.fatal("need one argument: Input Edge Text file path");
      return -1;
    }
    String inputfolder = arg0[0];
    Job job = new Job(getConf(), "Edge Text File Process:" + inputfolder);

    job.setJarByClass(EdgeFileProcessJob.class);
    job.setNumReduceTasks(0);// no Reducer
    job.setMapperClass(EdgeFileProcessMapper.class);
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new EdgeFileProcessJob(), args);
  }
